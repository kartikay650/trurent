...
export function filterListings(listings, filters = {}) {
  return listings.filter((l) => {
    ...
    // bhk: accept number OR array of numbers. The value 5 means "5 or more"
    // because the form/filter chip is "5+ BHK"; we never carry a "6 BHK"
    // filter explicitly. A listing with bhk >= 5 matches a [5] filter.
    if (filters.bhk != null) {
      const allowed = Array.isArray(filters.bhk)
        ? filters.bhk.map(Number)
        : [Number(filters.bhk)];
      const wantsFivePlus = allowed.includes(5);
      const exactMatch = allowed.includes(l.bhk);
      const plusMatch = wantsFivePlus && l.bhk >= 5;
      if (!exactMatch && !plusMatch) return false;
    }

    // listingType: exact match. entire_flat / room / pg.
    if (filters.listingType && l.listingType !== filters.listingType) return false;

    // postedWithinDays: cutoff on postedAt. Tolerant of missing values
    // (treats them as "old", so they get filtered out when a window is set).
    if (filters.postedWithinDays != null) {
      const cutoff = Date.now() - Number(filters.postedWithinDays) * 86400 * 1000;
      const postedMs = l.postedAt ? new Date(l.postedAt).getTime() : 0;
      if (!postedMs || postedMs < cutoff) return false;
    }
